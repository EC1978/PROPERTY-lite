...
export default function SettingsNav() {
    ...
    return (
        <aside className="w-full md:w-64 flex-shrink-0 flex md:flex-col gap-2 overflow-x-auto pb-4 md:pb-0 sticky top-[72px] md:top-0 z-30 bg-white dark:bg-[#050505] md:bg-transparent -mx-6 px-6 md:mx-0 md:px-0 scrollbar-hide">
            {navigation.map((item) => {
                ...
                return (
                    <Link
                        key={item.name}
                        href={item.href}
                        className={`flex items-center gap-2.5 px-4 py-2.5 rounded-full md:rounded-2xl transition-all font-bold text-[13px] whitespace-nowrap shrink-0 border transition-all duration-200 ${isActive

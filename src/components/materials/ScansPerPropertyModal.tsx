...
export default function ScansPerPropertyModal({
    ...
    return (
        <div className="fixed inset-0 z-[1000000] flex items-center justify-center p-4">
            ...
            <div className="relative bg-white dark:bg-[#0A0A0A] w-full max-w-2xl rounded-[2rem] md:rounded-[3rem] border border-gray-200 dark:border-white/5 shadow-[0_0_100px_rgba(0,0,0,0.5)] overflow-hidden animate-in fade-in zoom-in duration-300">
                {/* Header Section */}
                <div className="p-5 md:p-8 pb-4">
                    <div className="flex justify-between items-start mb-6">
                        <div>
                            <div className="flex items-center gap-3 mb-2">
                                <div className="size-8 md:size-10 rounded-2xl bg-emerald-500/10 flex items-center justify-center text-emerald-500 border border-emerald-500/20 shrink-0">
                                    <span className="material-symbols-outlined text-[20px] md:text-[24px]">analytics</span>
                                </div>
                                <h2 className="text-xl md:text-3xl font-black text-gray-900 dark:text-white tracking-tight">Preview Insights</h2>
                            </div>
                            <p className="text-[10px] md:text-[11px] font-black text-emerald-500 uppercase tracking-[0.2em] ml-11 md:ml-13">{materialName}</p>
                        </div>
                        <button onClick={onClose} className="size-10 md:size-12 shrink-0 rounded-2xl bg-gray-100 dark:bg-white/5 flex items-center justify-center text-gray-500 hover:text-gray-900 dark:hover:text-white transition-all hover:rotate-90">
                            <span className="material-symbols-outlined text-[20px] md:text-[24px]">close</span>
                        </button>
                    </div>

                    {/* Quick Stats Cards */}
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 md:gap-4 mb-4 md:mb-8">
                        <div className="p-4 md:p-5 bg-gray-50 dark:bg-white/[0.03] rounded-2xl md:rounded-[2rem] border border-gray-100 dark:border-white/5">
                            <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Totaal Scans</p>
                            <p className="text-xl md:text-2xl font-black text-gray-900 dark:text-white">{scans.length}</p>
                        </div>
                        <div className="p-4 md:p-5 bg-gray-50 dark:bg-white/[0.03] rounded-2xl md:rounded-[2rem] border border-gray-100 dark:border-white/5">
                            <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Unieke Hits</p>
                            <p className="text-xl md:text-2xl font-black text-emerald-500">{totalUnique}</p>
                        </div>
                        <div className="col-span-2 sm:col-span-1 p-4 md:p-5 bg-gray-50 dark:bg-white/[0.03] rounded-2xl md:rounded-[2rem] border border-gray-100 dark:border-white/5">
                ...
                {/* Main Content Area */}
                <div className="px-5 md:px-8 pb-5 md:pb-8 overflow-y-auto max-h-[60vh] md:max-h-[50vh] no-scrollbar">
                    {isLoading ? (
                        ...
                        <div className="space-y-4">
                            ...
                            {groupedScans.map((group) => (
                                <div
                                    ...
                                    className="p-4 md:p-6 bg-white dark:bg-white/[0.02] rounded-3xl md:rounded-[2.5rem] border border-gray-100 dark:border-white/5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 group hover:border-[#0df2a2]/30 transition-all hover:bg-emerald-500/[0.02]"
                                >
                                    <div className="min-w-0 flex-1">
                                        <div className="flex items-center gap-3 mb-2 md:mb-3">
                                            <div className="size-8 rounded-xl bg-gray-100 dark:bg-white/5 flex items-center justify-center text-gray-400 group-hover:text-emerald-500 transition-colors shrink-0">
                                                <span className="material-symbols-outlined text-[18px]">home_work</span>
                                            </div>
                                            <h3 className="font-bold text-gray-900 dark:text-white truncate text-base md:text-lg tracking-tight">{group.address}</h3>
                                        </div>
                                        <div className="flex flex-wrap items-center gap-x-4 gap-y-2 sm:ml-11">
                                    ...
                                    <button
                                        ...
                                        className="size-10 md:size-12 shrink-0 self-end sm:self-auto rounded-2xl bg-gray-50 dark:bg-white/5 text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 transition-all flex items-center justify-center relative border border-transparent hover:border-red-200 dark:hover:border-red-500/20 shadow-sm"
                                    ...
                                    >
                                        {isResetting === group.propertyId ? (
                                            ...
                                            <span className="material-symbols-outlined text-[20px] md:text-[22px]">history</span>
                                        )}
                                    </button>
                                </div>
                            ))}
                ...
                {/* Footer Section */}
                <div className="p-5 md:p-8 pt-4 md:pt-4 bg-gray-50/50 dark:bg-white/[0.01] border-t border-gray-100 dark:border-white/5">
                    <button
                        onClick={onClose}
                        className="w-full bg-gray-900 dark:bg-white text-white dark:text-black font-black py-4 md:py-5 rounded-[1.25rem] md:rounded-[1.5rem] transition-all active:scale-[0.98] text-[12px] md:text-sm uppercase tracking-[0.2em] shadow-xl shadow-black/10 hover:shadow-emerald-500/10"
                    >
                        Dashboard Sluiten
                    </button>
                </div>

    ...
    return (
        <div className="w-full max-w-5xl mx-auto space-y-8 animate-in fade-in zoom-in duration-500 pb-12">
            ...
            {error ? (
                ...
                <>
                    {/* Desktop Table */}
                    <div className="hidden sm:block -mx-4 sm:mx-0 bg-white dark:bg-[#1C1C1E]/50 border-y sm:border border-gray-200 dark:border-white/5 sm:rounded-2xl overflow-hidden shadow-sm dark:shadow-none backdrop-blur-xl">
                                        ...
                                        <tr>
                                            <td colSpan={4} className="px-6 py-12 text-center text-slate-500 dark:text-gray-500">
                                                <div className="flex flex-col items-center justify-center gap-3">
                                                    ...
                                                    <button
                                                        onClick={() => setIsInviteModalOpen(true)}
                                                        className="text-[#0df2a2] hover:underline mt-1 text-sm font-medium flex items-center gap-1 mx-auto"
                                                    >
                                                        <UserPlus className="w-4 h-4" /> Nodig uit
                                                    </button>
                                                </div>
                                            </td>
                                        </tr>
                                            ...
                                            <tr key={member.id} className="hover:bg-slate-50 dark:hover:bg-white/[0.02] transition-colors group">
                                                ...
                                                <td className="px-6 py-4">
                                                    {member.status === 'Active' ? (
                                                        ...
                                                        <span className="inline-flex items-center gap-1.5 text-sm text-amber-500 dark:text-amber-400 bg-amber-500/10 dark:bg-amber-400/10 px-2.5 py-1 rounded-full border border-amber-500/20 dark:border-amber-400/20">
                                                ...
                                                <td className="px-6 py-4 text-right">
                                                    <button
                                                        ...
                                                        className="p-2 text-slate-400 dark:text-gray-500 hover:text-red-500 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-400/10 rounded-lg transition-colors opacity-0 group-hover:opacity-100 focus:opacity-100 disabled:opacity-50"
                                                        title="Verwijderen"
                                                    >
                    ...
                    <div className="sm:hidden space-y-4">
                        {initialMembers.length === 0 ? (
                            <div className="p-8 text-center bg-white dark:bg-[#1C1C1E]/50 border border-gray-200 dark:border-white/5 rounded-2xl shadow-sm dark:shadow-none">
                                <Users className="w-10 h-10 mx-auto text-slate-300 dark:text-gray-600 mb-3" />
                                <p className="text-sm text-slate-500 dark:text-gray-400 mb-3">U heeft nog geen teamleden.</p>
                                <button
                                    onClick={() => setIsInviteModalOpen(true)}
                                    className="text-[#0df2a2] text-sm font-medium hover:underline flex flex-col items-center gap-1 mx-auto"
                                >
                                    <UserPlus className="w-5 h-5" />
                                    Nodig uit
                                </button>
                            ...
                            initialMembers.map((member) => (
                                <div key={member.id} className="bg-white dark:bg-[#1C1C1E]/50 border border-gray-200 dark:border-white/5 rounded-xl p-4 shadow-sm dark:shadow-none flex flex-col gap-4">
                                    <div className="flex items-start justify-between">
                                        ...
                                        <button
                                            onClick={() => handleDelete(member.id)}
                                            disabled={deletingId === member.id}
                                            className="p-2 -m-2 text-slate-400 dark:text-gray-500 hover:text-red-500 dark:hover:text-red-400 bg-transparent shrink-0 disabled:opacity-50"
                                        >
                                            {deletingId === member.id ? (
                                                <Loader2 className="w-4 h-4 animate-spin" />
                                            ) : (
                                    ...
                                    <div className="flex items-center justify-between pt-3 border-t border-gray-100 dark:border-white/5">
                                        <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-slate-100 dark:bg-white/5 text-xs font-medium text-slate-700 dark:text-gray-300 border border-gray-200 dark:border-white/5">
                                            <Shield className="w-3 h-3" />
                                            {member.role}
                                        </div>
                                        <div>
                                            {member.status === 'Active' ? (
                                                <span className="inline-flex items-center gap-1.5 text-xs text-[#0df2a2] bg-[#0df2a2]/10 px-2.5 py-1 rounded-full border border-[#0df2a2]/20">
                                                    <CheckCircle2 className="w-3 h-3" />
                                                    Actief
                                                </span>
                                            ) : (
                                                <span className="inline-flex items-center gap-1.5 text-xs text-amber-500 dark:text-amber-400 bg-amber-500/10 dark:bg-amber-400/10 px-2.5 py-1 rounded-full border border-amber-500/20 dark:border-amber-400/20">

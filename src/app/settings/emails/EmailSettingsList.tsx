...
export default function EmailSettingsList({ initialSettings }: { initialSettings: EmailSetting[] }) {
    ...
    return (
        <div className="space-y-4">
            {settings.length === 0 ? (
                <div className="text-center text-gray-400 py-10 bg-[#121212] rounded-2xl border border-white/10">
                ...
                settings.map(setting => (
                    <div
                        key={setting.id}
                        className="flex flex-col md:flex-row items-start md:items-center justify-between p-6 bg-[#121212] rounded-2xl border border-white/10 shadow-lg gap-4 transition-all hover:bg-[#1a1a1a]"
                    >
                        <div className="flex gap-4 items-center">
                            ...
                            <div>
                                <h3 className="text-white font-semibold text-[16px]">{setting.name}</h3>
                                <p className="text-gray-400 text-sm mt-1">Onderwerp: {setting.subject}</p>
                            </div>
                        </div>
                        ...
                        <div className="flex items-center gap-4 mt-2 md:mt-0 w-full md:w-auto justify-end">
                            <button
                                ...
                                className="px-4 py-2 text-sm font-medium text-white bg-white/5 hover:bg-white/10 rounded-lg transition-colors flex items-center gap-2 border border-white/10"
                            >
